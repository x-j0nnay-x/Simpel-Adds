...
public class HomeWandNeoForge extends Item {

    private int cooldown;
    private boolean oncooldown;
    private int maxcooldown = 240;

    public HomeWandNeoForge(int maxUese) {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        if(!level.isClientSide) {
            ...
            if (player.isCrouching()) {
                if (itemStack.getComponents().get(ModDataComponentTypesNeoForge.HOMEWAND_COMPNENTS.get()) == null) {
                    setData(player);
            ...
            if (!this.oncooldown && !player.isCrouching()) {
                if (isUseable(itemStack)) {
                    if (itemStack.getComponents().get(ModDataComponentTypesNeoForge.HOMEWAND_COMPNENTS.get()) != null && itemStack.getComponents().get(DataComponents.CUSTOM_DATA) != null) {
    ...
    private void setData(Player player){
        ...
        itemStack.set(ModDataComponentTypesNeoForge.HOMEWAND_COMPNENTS.get(), new BlockPos(player.getBlockX(), player.getBlockY(), player.getBlockZ()));
    ...
    private void removeData(Player player){
        ItemStack itemStack = player.getItemInHand(InteractionHand.MAIN_HAND);
        itemStack.remove(ModDataComponentTypesNeoForge.HOMEWAND_COMPNENTS.get());
        itemStack.remove(DataComponents.CUSTOM_DATA);
    }

    private void tellaport(Player player) {
        ItemStack itemStack = player.getItemInHand(InteractionHand.MAIN_HAND);
        double X = (double) itemStack.getComponents().get(ModDataComponentTypesNeoForge.HOMEWAND_COMPNENTS.get()).getX() + 0.5;
        double Y = (double) itemStack.getComponents().get(ModDataComponentTypesNeoForge.HOMEWAND_COMPNENTS.get()).getY();
        double Z = (double) itemStack.getComponents().get(ModDataComponentTypesNeoForge.HOMEWAND_COMPNENTS.get()).getZ() + 0.5;
    ...
    public void appendHoverText(ItemStack $$0, TooltipContext $$1, List<Component> $$2, TooltipFlag $$3) {
        if($$0.getComponents().get(ModDataComponentTypesNeoForge.HOMEWAND_COMPNENTS.get()) != null){
            String title = Component.translatable("item.simpeladdmod.homewand.homelocal").getString();
            $$2.add(Component.literal(title + $$0.get(ModDataComponentTypesNeoForge.HOMEWAND_COMPNENTS.get())));
