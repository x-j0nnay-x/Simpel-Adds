...
public class FabricGrindFactoryMenu extends AbstractContainerMenu {
    public  final FabricGrindFactoryBlockEntity blockEntity;
    private final Container inventory;
    private final Level level;
    private final ContainerData data;

    public FabricGrindFactoryMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData){
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(13));
    }
    public FabricGrindFactoryMenu(int pContainerID, Inventory inv, BlockEntity entity, ContainerData data){
        super(ModMenuTypeFabric.GRIND_FACTORY_MENU, pContainerID);
        checkContainerSize(inv, 18);
        blockEntity = ((FabricGrindFactoryBlockEntity) entity);
