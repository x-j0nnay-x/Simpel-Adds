...
/*
        private static Codec<List<Ingredient>> validateAmount(Codec<Ingredient> delegate, int max) {
            ...
            return ExtraCodecs.nonEmptyHolderSet(ExtraCodecs.intervalCodec(
        ...
        public static final Codec<GrinderRecipe> CODEC = RecordCodecBuilder.create(in -> in.group(
                ...
                ItemStack.ITEM_NON_AIR_CODEC.fieldOf("output").forGetter(r -> r.output.getItemHolder())
        ).apply(in,  (ingredients, result) -> {
            var nnIngredients = NonNullList.<Ingredient>create();
            nnIngredients.addAll(ingredients);
            return new GrinderRecipe(nnIngredients, result.get().getDefaultInstance());
        }));
