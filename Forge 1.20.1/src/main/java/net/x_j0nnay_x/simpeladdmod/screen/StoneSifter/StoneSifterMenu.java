...
public class StoneSifterMenu extends AbstractContainerMenu {
    public  final StoneSifterBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public StoneSifterMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData){
    ...
    public StoneSifterMenu(int pContainerID, Inventory inv, BlockEntity entity, ContainerData data){
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(iItemHandler -> {
            ...
            this.addSlot(new SlotItemHandler(iItemHandler, StoneSifterBlockEntity.GRINDERSLOT, 16, 17){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return ModItems.GRINDERHEAD.get() == stack.getItem();
                }
            });
            this.addSlot(new SlotItemHandler(iItemHandler, StoneSifterBlockEntity.COPPERSLOT, 124, 17){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return false;
                }
            });
            this.addSlot(new SlotItemHandler(iItemHandler, StoneSifterBlockEntity.IRONSLOT, 142, 17){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return false;
                }
            });
            this.addSlot(new SlotItemHandler(iItemHandler, StoneSifterBlockEntity.GOLDSLOT, 124, 35){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return false;
                }
            });
            this.addSlot(new SlotItemHandler(iItemHandler, StoneSifterBlockEntity.REDSTONESLOT, 142, 35){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return false;
                }
            });
            this.addSlot(new SlotItemHandler(iItemHandler, StoneSifterBlockEntity.QUARTZSLOT, 124, 53){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return false;
                }
            });
            this.addSlot(new SlotItemHandler(iItemHandler, StoneSifterBlockEntity.DIMOANDSLOT, 142, 53){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return false;
                }
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.STONE_SHIFTER.get());
    }
