...
public class FabricBlockFactoryMenu extends AbstractContainerMenu {
    ...
    public FabricBlockFactoryMenu(int pContainerID, Inventory inv, BlockEntity entity, ContainerData data){
        ...
        addPlayerHotbar(inv);
            ...
            this.addSlot(new Slot(this.inventory, FabricBlockFactoryBlockEntity.WATERSLOT, 34, 53){
                @Override
                public boolean mayPlace(ItemStack stack) {
                   return Items.WATER_BUCKET == stack.getItem();
                }
            });
            this.addSlot(new Slot(this.inventory, FabricBlockFactoryBlockEntity.COBBLESLOT, 52, 35){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return false;
                }
            });
            this.addSlot(new Slot(this.inventory, FabricBlockFactoryBlockEntity.GRAVALSLOT, 70, 35){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return false;
                }
            });
            this.addSlot(new Slot(this.inventory, FabricBlockFactoryBlockEntity.SANDSLOT, 88, 35){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return false;
                }
            });
            this.addSlot(new Slot(this.inventory, FabricBlockFactoryBlockEntity.OBSIDIANSLOT, 106, 35){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return false;
                }
