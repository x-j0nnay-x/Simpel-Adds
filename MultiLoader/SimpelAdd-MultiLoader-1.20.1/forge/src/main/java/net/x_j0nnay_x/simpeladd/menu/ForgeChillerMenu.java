...
public class ForgeChillerMenu extends AbstractContainerMenu {
    public  final ForgeChillerBlockEntity blockEntity;
    private final Level level;
    private FluidStack fluidStack;
    private final ContainerData data;

    public ForgeChillerMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData){
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(5));
    }
    public ForgeChillerMenu(int pContainerID, Inventory inv, BlockEntity entity, ContainerData data){
        super(ModMenuTypeForge.Chiller_MENU.get(), pContainerID);
        checkContainerSize(inv, 3);
        blockEntity = ((ForgeChillerBlockEntity) entity);
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(iItemHandler -> {
            ...
            this.addSlot(new SlotItemHandler(iItemHandler, ForgeChillerBlockEntity.CHILLINGSLOT, 16, 53){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return stack.is(ModTags.Items.CHILLING);
                }
            });
            this.addSlot(new SlotItemHandler(iItemHandler, ForgeChillerBlockEntity.OUTPUTSLOT, 133, 26){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return false;
                }
            });
            this.addSlot(new FluidContainerSlot(iItemHandler, ForgeChillerBlockEntity.WATERSLOT, 52, 53){
                @Override
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlockRegForge.CHILLER.get());
    }
