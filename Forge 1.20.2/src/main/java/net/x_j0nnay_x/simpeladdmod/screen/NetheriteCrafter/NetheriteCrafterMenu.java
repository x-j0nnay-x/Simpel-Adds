...
public class NetheriteCrafterMenu extends AbstractContainerMenu {
    ...
    public NetheriteCrafterMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData){
    ...
    public NetheriteCrafterMenu(int pContainerID, Inventory inv, BlockEntity entity, ContainerData data){
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(iItemHandler -> {
            ...
            this.addSlot(new SlotItemHandler(iItemHandler, NetheriteCrafterBlockEntity.SCRAPSLOT, 25, 35){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return stack.is(Items.NETHERITE_SCRAP);
                }
            });
            this.addSlot(new SlotItemHandler(iItemHandler, NetheriteCrafterBlockEntity.OUTPUTSLOT, 124, 35){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return false;
                }
            });
            this.addSlot(new SlotItemHandler(iItemHandler, NetheriteCrafterBlockEntity.GOLDSLOT, 52, 35){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return stack.is(Items.GOLD_INGOT);
                }
            });
            this.addSlot(new SlotItemHandler(iItemHandler, NetheriteCrafterBlockEntity.BLAZESLOT, 79, 35){
                @Override
                public boolean mayPlace(ItemStack stack) {
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.NETHERITE_CRAFTER.get());
    }
