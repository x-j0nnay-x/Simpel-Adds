...
public class ChillerBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
        ...
        if (itemStack.hasCustomName() && (blockEntity = world.getBlockEntity(pos)) instanceof ChillerBlockEntity) {
            ((ChillerBlockEntity)blockEntity).getDisplayName();
        }
    }
    @Override
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof ChillerBlockEntity) {
                ItemScatterer.spawn(world, pos, (ChillerBlockEntity)blockEntity);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!world.isClient) {
            NamedScreenHandlerFactory screenHandlerFactory = ((ChillerBlockEntity) world.getBlockEntity(pos));
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.CHILLER,
